import {
  ...
  export const ControlPanelSection = ({
    ...
    const filterOptions = [
      ...
      { value: "enabled", label: "Enabled Only", icon: <Eye className="w-4 h-4 text-green-400" /> },
      {
        value: "disabled",
        label: "Disabled Only",
    ...
    const cpuName = "11th Gen Intel(R) Core(TM) i7-1185G7 @ 3.00GHz";
    ...
    return (
      <div className="flex items-center gap-3 w-full my-4">
        {/* Search */}
        <div className="flex items-center gap-2.5 rounded-[10px] border border-[#1e1e28] bg-core-grey800 px-3">
        ...
        <Dropdown
          options={sortOptions.map((option) => ({
            ...option,
            label: `${option.label} ${
              sortState === option.value ? (sortDirection === "asc" ? "↑" : "↓") : ""
            }`,
          }))}
          value={sortState}
        ...
        <Button
          ...
          className="flex items-center gap-2.5 px-3 py-[9px] h-auto rounded-[10px] border border-[#1e1e28] bg-core-grey800 text-white hover:bg-core-grey700 disabled:opacity-50"
        >
        ...
        {activeTab === "cpu" && (
          <Button
            variant="outline"
            className="flex items-center gap-2 px-3 py-1.5 text-xs h-auto max-w-[300px] truncate rounded-md border border-[#1e1e28] bg-core-grey800 text-white hover:bg-core-grey700"
          >
            <CpuIcon className="w-4 h-4" /> {cpuName}
          </Button>
        )}
      </div>
    );
