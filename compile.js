...
async function compileFileToBytecode(inputPath, outputJscPath) {
    console.log(
        `[compile] ${path.relative(process.cwd(), inputPath)} -> ${path.relative(process.cwd(), outputJscPath)}`,
    );
    await ensureDir(path.dirname(outputJscPath));
    bytenode.compileFile({
        filename: inputPath,
        output: outputJscPath,
        electron: true,
...
async function writeLoader(loaderPath, jscRelativePath, isPreload = false) {
    const content =
        `// Auto-generated loader for bytenode-compiled file\n` +
        `require('bytenode');\n` +
        `const path = require('path');\n` +
